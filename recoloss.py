...
def vsup256infer(U,x,la):
   
    Uout,uo,foz,zs1,size1 =  U(x)
    Uf=foz.transpose(0,1)
    uo[:, :5] = F.conv3d(
        uo[:, :5].float(),
        DK1.expand(5, 1, *DK1.shape[2:]),  # shape [5,1,7,7,1]
        padding=(3,3,0),
        groups=5
    )
    # uu=(Uout.argmax(1)==1)
...
def run_unet_on_patches_infer(feature_extract_net, volume_tensor, label_tensor):
    """
    Sliding-window inference over the full volume via patches.
    ...
    p, f, zs, s, lb, uout : tuple
        ...
        - uout: Tensor[H,W,Z] aggregated UNet prediction
    """
    U,x,l=feature_extract_net, volume_tensor, label_tensor
    u,c,pl1,pl2,pl3=x.shape
    ax=pl3
    #print(ax)
    if pl3<32:
    ...
    for x1 in range(xm):
        for y1 in range(ym):
            for z1 in range(zm):
                ...
                with torch.no_grad():
                    p1,f1,s1,zs1,lb1,ar,uo=  vsup256infer(U,x[:,:,v1:v2,v3:v4,v5:v6],l[:,v1:v2,v3:v4,v5:v6])  
