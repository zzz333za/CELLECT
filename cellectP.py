...
def vsup256(U,x,la):
    ...
    uc=((kflb(uo[:,:].max(1)[0]).to(device).max(1)[0]==uo[:,4]))*(Uout.argmax(1)==1)
    uc[:,:3]=0
    uc[:,-3:]=0
    uc[:,:,:3]=0
    uc[:,:,-3:]=0
...
def track(inputs,in2,in3,zratio,PA,U,EX,EN):
    device = inputs.device
    o1={}
    o2={}
    with torch.no_grad():
                p1,f1,zs1,s1,lb1,uout =  bvsup256(U,(PA(torch.cat([inputs,in2],1))),inputs[:,0]*0)
