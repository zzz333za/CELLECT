...
def vsup256(U, x, la):
    ...
    uc = ((kflb(uo[:, :].max(1)[0]).to(device).max(
...
#frame 1
        if step > 0:
            ...
            for jk in range(5):
                ts1 = s1[q[:, jk]]
                es.append(torch.cat([(ts1 -
                                      s1).unsqueeze(1), (ts1 /
                                                         (s1 +
                                                          1)).unsqueeze(1), epyy[:, jk].unsqueeze(1), (ts1 -
                                                                                                       epyy[:, jk]).unsqueeze(1), ((ts1 -
                                                                                                                                    epyy[:, jk]) > 0).float().unsqueeze(1), (ts1 /
                                                                                                                                                                             (epyy[:, jk] +
                                                                                                                                                                              0.0001)).unsqueeze(1)], 1).unsqueeze(1))
                tzs1 = zs1[q[:, jk]]
                ezs.append(torch.cat([zs1.unsqueeze(1),
                                      tzs1.unsqueeze(1),
                                      torch.cat([((zs1[:,
                                                       0] > 0.5) * (tzs1[:,
                                                                         1] > 0.5)).unsqueeze(1).float(),
                                                 ((zs1[:,
                                                       0]) * (tzs1[:,
                                                                   1])).unsqueeze(1)],
                                                1).unsqueeze(1)],
                                                                                                   ...
                                                                                                   epyy[:, jk]).unsqueeze(1), ((ts1 -
                                                                                                                                epyy[:, jk]) > 0).float().unsqueeze(1), (ts1 /
                                                                                                                                                                         (epyy[:, jk] +
                                                                                                                                                                          0.0001)).unsqueeze(1)], 1).unsqueeze(1))
            tzs1 = zs2[q[:, jk]]
            ezs.append(torch.cat([zs2.unsqueeze(1),
                                  tzs1.unsqueeze(1),
                                  torch.cat([((zs2[:,
                                                   0] > 0.5) * (tzs1[:,
                                                                     1] > 0.5)).unsqueeze(1).float(),
